...
class _DeliverRescheduleViewState extends State<DeliverRescheduleView> {
  ...
  Future<void> confirmReschedule(BuildContext context) async {
    ...
    final response = await http.put(
      Uri.parse(
          '${EnvironmentVariables.baseUrl}pedido/reprogramar/${deliver!.id}?idUser=${authBloc2.userId}&motivo=$reason'),
    ...
    if (map['statusCode'] == 200) {
      ...
      for (var element in deliverPhotos!) {
        final request = http.MultipartRequest(
          'PUT',
          Uri.parse(
              '${EnvironmentVariables.baseUrl}pedido/imagenReprogramacion/${deliver!.id}?user_id=${authBloc2.userId}&reprogramacion_id=$idReschedule'),
        );
        request.files.add(
          http.MultipartFile(
            'imagen',
